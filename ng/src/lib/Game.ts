...
export class Game {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    // Initial maze via Laby (deterministic seed)
    this.cells = this.generateCells(25, 17);
  ...
  private update(dt: number) {
    ...
    if (Math.hypot(dx, dy) < 0.5) {
      this.level += 1;
      const baseW = 25, baseH = 17;
      const add = Math.min(20, this.level * 2);
      this.cells = this.generateCells(baseW + add, baseH + add);
      this.placePlayerAndGoal();
    }
    // Occasionally regenerate maze (same seed) to demo activity
    if (this.tickCount % 600 === 0) {
      this.cells = this.generateCells(this.cells[0]?.length ?? 25, this.cells.length || 17);
    }
  }
  ...
  private generateCells(cols: number, rows: number): number[][] {
    // Laby outputs a grid of size (w*2-1) x (h*2-1). Passing odd sizes
    // yields the same dimensions back. We keep API as cols/rows here.
    const laby = new Laby(cols, rows, Game.SEED);
